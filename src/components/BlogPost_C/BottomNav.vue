...
<template>
  <div class="post-nav">
    ...
    <button class="nav-btn" style="height: 3em; width: 3em;">
      <ThemeToggle />
    </button>
    ...
    <div class="column">
      <div class="row">
        <h3 v-show="isHoveringTopic">
          Posts in Category
        </h3>
        <div class="grow"></div>
        <button
          class="nav-btn"
          @click="emit('previous-topic')"
          :disabled="!hasPreviousTopic"
          style="transform: scaleX(-1);"
          @mouseenter="isHoveringTopic = true"
          @mouseleave="isHoveringTopic = false"
        >
            <nextPostInTopic style="height: 100%;"/>
        </button>
        <button
          class="nav-btn"
          @click="emit('next-topic')"
...
.vertical-arrow {
  height: 100%;
  /* Make SVG fill the accent container height */
  width: auto;
}
.horozontal-arrow:deep(svg) {
  height: 100%;
  /* Make SVG fill the accent container height */
  width: auto;
...
.horozontal-arrow.flipped:deep(svg) {
  transform: scaleX(-1);
...
/* Ensure h3 elements don't take space when hidden */
h3 {
  margin: 0;
  white-space: nowrap;
